...
export function simulateTrace(options: {
  ...
  const samples: TraceSimulationResult["samples"] = [];
  const visibleTimeline: TraceSimulationResult["visibleTimeline"] = [
    { timestampMs: 0, visibleChars: 0 },
  ];
  const fullTextTimeline: TraceSimulationResult["fullTextTimeline"] = [
  ...
  const finalEventTimeMs =
    events.length > 0 ? events[events.length - 1].timeMs : 0;
  ...
  while (
    ...
    if (result.nextSliceEnd > visibleText.length) {
      ...
      codeFenceState = advanceCodeFenceState(
        codeFenceState,
        nextText.slice(visibleText.length),
      );

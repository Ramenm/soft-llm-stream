...
import type { SchedulerHandle } from "./core-utils.js";
import {
...
export function createSoftLlmChatStream(
  options: CreateSoftLlmChatStreamOptions,
): SoftLlmStreamStore {
...
export function createSoftLlmStream(
  options: CreateSoftLlmStreamOptions,
): SoftLlmStreamStore {
  const listeners = new Set<() => void>();
  let snapshot = { ...EMPTY_SNAPSHOT };
  let controller: AbortController | null = null;
  let startPromise: Promise<StreamSnapshot> | null = null;
  let scheduled: SchedulerHandle = null;
  let visibilityUnsubscribe: (() => void) | null = null;
  let lifecycleId = 0;
  let renderedCodeFenceState = createCodeFenceState();

  const locale = resolveLocale(options.locale);
  const tuning = mergeRevealTuning(
    options.debug?.tuning,
    options.revealProfile ?? "balanced",
  );
  ...
  const shouldBypassReveal = () =>
    options.reveal === false || prefersReducedMotion();
  const isLifecycleCurrent = (value: number) => lifecycleId === value;
  ...
  const setSnapshot = (patch: Partial<StreamSnapshot>) => {
  ...
  const setRenderedText = (nextText: string) => {
    ...
    if (nextText.startsWith(snapshot.text)) {
      renderedCodeFenceState = advanceCodeFenceState(
        renderedCodeFenceState,
        nextText.slice(snapshot.text.length),
      );
    } else {
  ...
  const finalizeIfSettled = (runId = lifecycleId) => {
    ...
    if (
      snapshot.fullText.length === snapshot.text.length &&
  ...
  const applyNextFullText = (runId: number, nextFullText: string, at: number) => {
    const nextStatus =
      snapshot.status === "connecting" ? "streaming" : snapshot.status;
  ...
  const applyAppendedText = (runId: number, text: string, at: number) => {
    if (!text) {
      return;
    }
  ...
  const syncReplacedText = (nextText: string) => {
    const nextStatus =
      snapshot.status === "connecting" ? "streaming" : snapshot.status;
  ...
  const applyReplacedText = (runId: number, nextText: string, at: number) => {
  ...
  const runRevealStep = (runId: number, at: number) => {
    if (!isLifecycleCurrent(runId)) {
    ...
    if (result.nextSliceEnd > snapshot.text.length) {
      appendRenderedText(
        snapshot.fullText.slice(snapshot.text.length, result.nextSliceEnd),
      );
    }
  ...
  const isTerminalSnapshot = (value: StreamSnapshot) =>
    value.status === "done" || value.status === "stopped" || value.status === "error";
  ...
  const waitForTerminalSnapshot = () => {
    ...
    return new Promise<StreamSnapshot>((resolve) => {
      const listener = () => {
        if (
          isTerminalSnapshot(snapshot) &&
          (!snapshot.hasBacklog || snapshot.status !== "done")
  ...
  const start = async () => {
    ...
    const run = async () => {
      try {
        ...
        const { source: preparedSource, adapter } = await prepareSourceAdapter(
          resolvedSource,
          options.adapter,
    ...
    const currentStartPromise = run().then(async (currentSnapshot) => {
      ...
      if (
        isTerminalSnapshot(currentSnapshot) &&
  ...
  const store: SoftLlmStreamStore = {

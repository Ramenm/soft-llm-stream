...
export const DEFAULT_REVEAL_TUNING: RevealTuning = {
...
export const FAST_FIRST_REVEAL_TUNING: RevealTuning = {
...
export const SOFT_FINISH_REVEAL_TUNING: RevealTuning = {
  ...DEFAULT_REVEAL_TUNING,
...
export function resolveRevealTuningPreset(
  presetName: RevealTuningPresetName = "balanced",
): RevealTuning {
  return REVEAL_TUNING_PRESETS[presetName] ?? DEFAULT_REVEAL_TUNING;
}

function mergeModeMap(
  current: Record<RevealMode, number>,
  patch?: Partial<Record<RevealMode, number>>,
) {
  if (!patch) {
    return current;
  }
...
export function mergeRevealTuning(
  overrides?: Partial<RevealTuning>,
  presetName: RevealTuningPresetName = "balanced",
): RevealTuning {
  ...
  return {
    ...
    budgetBankFrames: mergeModeMap(
      baseTuning.budgetBankFrames,
      overrides.budgetBankFrames,
    ),
    maxStepChars: mergeModeMap(baseTuning.maxStepChars, overrides.maxStepChars),
    stepRateMultipliers: mergeModeMap(
      baseTuning.stepRateMultipliers,
      overrides.stepRateMultipliers,
    ),
    boundaryOvershootChars: mergeModeMap(
      baseTuning.boundaryOvershootChars,
      overrides.boundaryOvershootChars,
    ),
  };
}

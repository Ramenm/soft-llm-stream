...
export function advanceCodeFenceState(state, appendedText) {
    if (!appendedText) {
        return state;
    }
...
export function getCodeFenceState(text) {
...
function findCodePointBoundary(text, targetLength) {
    if (!text) {
        return 0;
    }
...
function isSimpleGraphemeBoundary(text, targetLength) {
    ...
    return !(COMPLEX_BOUNDARY_RE.test(boundaryWindow) ||
        EMOJI_MODIFIER_RE.test(boundaryWindow));
}
function findLooseSegmentBoundary(text, targetLength, direction) {
    const searchWindow = text.slice(0, Math.min(text.length, Math.max(targetLength * BOUNDARY_TUNING.segmenterWindowFactor, BOUNDARY_TUNING.punctuationWindowMinChars)));
    ...
    if (direction === "before") {
        ...
        if (punctuationBoundary?.index != null &&
    ...
    const punctuationBoundary = punctuation.find((part) => part.index != null && part.index + 1 >= targetLength);
    if (punctuationBoundary?.index != null &&
        punctuationBoundary.index + 1 <=
...
export function findGraphemeBoundary(text, targetLength, locale) {
    if (!text) {
        return 0;
    }
    if (text.length <= targetLength) {
    ...
    if (segmenter) {
        let consumed = 0;
        const segmentWindow = text.slice(0, Math.min(text.length, Math.max(targetLength + BOUNDARY_TUNING.segmenterWindowPaddingChars, Math.max(targetLength * 2, targetLength + 8))));
...
function findLineBoundary(text, targetLength, direction = "after") {
    if (text.length <= targetLength) {
        return text.length;
    }
    if (direction === "before") {
...
function findSegmentBoundary(text, targetLength, locale, direction = "after") {
    if (!text) {
        return 0;
    }
    if (text.length <= targetLength) {
    ...
    if (segmenter) {
        const segmentWindow = text.slice(0, Math.min(text.length, Math.max(targetLength * BOUNDARY_TUNING.segmenterWindowFactor, targetLength + BOUNDARY_TUNING.segmenterWindowPaddingChars)));
        let consumed = 0;
...
export function getSliceSearchWindowChars(maxChars) {
    const safeMaxChars = Math.max(1, maxChars);
    return Math.max(BOUNDARY_TUNING.punctuationWindowMinChars, Math.ceil(safeMaxChars * BOUNDARY_TUNING.segmenterWindowFactor), safeMaxChars + BOUNDARY_TUNING.segmenterWindowPaddingChars);
}
export function chooseSliceLength(remaining, preferredChars, options) {
    if (preferredChars <= 0 || options.maxChars <= 0) {
        return 0;
    }
    const requestedMaxChars = Math.max(1, Math.min(options.maxChars, remaining.length));
    ...
    const overshootBudget = Math.max(0, Math.min(options.maxOvershootChars, hardLimit - exactBoundary));

...
function getQuantileFromSorted(values, ratio) {
    if (values.length === 0) {
        return 0;
    }
...
function getMedianFromSorted(values) {
    if (values.length === 0) {
        return 0;
    }
...
function getWinsorizedMean(values, sortedValues, lowerRatio = 0.1, upperRatio = 0.9) {
    if (values.length === 0) {
        return 0;
    }
    if (values.length < 3) {
...
export function createArrivalEstimator() {
    return {
        lastChunkAt: null,
        firstChunkChars: 0,
        samples: [],
...
export function resetArrivalEstimator(state) {
...
export function noteArrival(state, chunkText, at, tuning) {
    const chunkChars = typeof chunkText === "number" ? Math.max(0, chunkText) : chunkText.length;
    if (chunkChars <= 0) {
        return;
    }
...
function buildArrivalEstimateCache(state, tuning) {
    const gaps = new Array(state.samples.length);
    const chunkSizes = new Array(state.samples.length);
    let totalGapMs = 0;
    ...
    return {
        arrivalRateCharsPerMs: clamp(blendedRate, tuning.bootstrapMinRateCharsPerMs, tuning.estimatedRateMaxCharsPerMs),
        meanGapMs,
        jitterBaseMs,
        meanChunkChars,
...
export function getArrivalEstimate(state, frameMs, tuning) {
    if (state.samples.length === 0) {
        const meanChunkChars = Math.max(1, state.firstChunkChars || 1);
        const meanGapMs = tuning.bootstrapDefaultGapMs;
        const arrivalRateCharsPerMs = clamp(meanChunkChars / meanGapMs, tuning.bootstrapMinRateCharsPerMs, tuning.bootstrapMaxRateCharsPerMs);
        return {
    ...
    const predictedGapMs = Math.max(frameMs, state.cache.meanGapMs + jitterMs * tuning.gapJitterMultiplier);
    return {
